...
class Database
{
    ...
    private static function ensureBootstrapAdmin(string $dbPath): void
    {
        ...
        $email = mb_strtolower(trim((string)(getenv('CLIENTPAY_ADMIN_EMAIL') ?: self::LEGACY_ADMIN_EMAIL)));
        ...
        $stm = self::$pdo->prepare('INSERT INTO users(name,email,password_hash,role) VALUES(?,?,?,?)');
        $stm->execute([$name, $email, $hash, 'admin']);
    ...
    private static function runMigrations(): void
    {
        ...
        $migrations = [
            ...
            2026062701 => [
                'name' => 'production_safety_and_business_integrity',
                'up' => static function (): void {
                    self::ensureColumn('users', 'role', "TEXT NOT NULL DEFAULT 'admin'");
                    self::ensureColumn('clients', 'archived_at', 'TEXT');
                    self::ensureColumn('sites', 'archived_at', 'TEXT');
                    self::ensureColumn('invoices', 'paid_at', 'TEXT');
                    self::ensureColumn('invoices', 'payment_method', 'TEXT');
                    self::ensureColumn('invoices', 'payment_reference', 'TEXT');

                    self::$pdo->exec("UPDATE users SET role='admin' WHERE role IS NULL OR role NOT IN ('admin','operator')");

                    if (self::tableExists('plan_history')) {
                        self::$pdo->exec(
                            'DELETE FROM plan_history
                             WHERE id NOT IN (
                                 SELECT MAX(id) FROM plan_history GROUP BY site_id, effective_from
                             )'
                        );
                        self::$pdo->exec(
    ...
    private static function ensureDefaultTemplates(): void
    {
        $templates = [
            ['before_due', 'Pré-vencimento', 'Olá {client_name}! Lembrando que sua mensalidade do site {site_name} vence em {due_date}. Valor: R$ {amount}. Qualquer dúvida, estou à disposição.'],
        ...
        $stm = self::$pdo->prepare('INSERT OR IGNORE INTO templates(code,title,body,active) VALUES(?,?,?,1)');
        foreach ($templates as $template) {
            $stm->execute($template);
        }
    }

    private static function syncCurrentMonthlyFees(): void
    {
        $today = date('Y-m-d');
        $stm = self::$pdo->prepare(
            'UPDATE sites
             SET current_monthly_fee = COALESCE(
                 (SELECT ph.amount FROM plan_history ph
                  WHERE ph.site_id=sites.id AND ph.effective_from<=?
